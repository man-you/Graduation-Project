<template>
  ...
  >
    ...
    <div
      ...
      class="absolute inset-0 z-40 flex flex-col items-center justify-center bg-slate-900/5 backdrop-blur-sm"
    >
      <div
        class="flex flex-col items-center w-[90%] max-w-[320px] p-8 bg-white shadow-xl shadow-slate-200/40 rounded-3xl border border-slate-100"
      >
        <div class="bg-rose-50 p-3.5 rounded-2xl mb-5 text-rose-500">
          <PhWarningCircle width="32" height="32" weight="bold" />
        </div>

        <h3 class="text-slate-900 text-lg tracking-tight">无法加载图谱数据</h3>

        <p class="text-slate-500 text-xs mt-2.5 text-center leading-relaxed px-2">
          当前课程内容尚未准备就绪，或者知识点结构不完整。请等待教师更新内容。
        </p>
      </div>
    </div>
...
const getParentName = (node) => {
  if (!node || !currentLinks) return '无'
  const link = currentLinks.find(
    (l) => l.target && (l.target.id === node.id || l.target === node.id),
  )
  return link && link.source ? link.source.name : '无'
}

const getChildren = (node) => {
  if (!node || !currentLinks) return []
  return currentLinks
    .filter((l) => l.source && (l.source.id === node.id || l.source === node.id))
    .map((l) => l.target)
}
...
watch(
  ...
  (newData) => {
    ...
    if (newData && newData.nodes && newData.nodes.length > 0) {
      ...
      // 容错：如果 links 不存在，初始化为空数组
      const linksCopy = newData.links ? JSON.parse(JSON.stringify(newData.links)) : []
...
const initGraph = (nodes, links) => {
  ...
  simulation = d3
    ...
    .force(
      ...
      d3
        .forceLink(links)
        .id((d) => d.id)
        .distance((d) => (d.target.level === 4 ? 60 : 120)),
    )
    .force(
      'charge',
  ...
  const link = g
    ...
    .join('line')
    .attr('stroke-width', (d) => {
      const targetLevel = d.target ? d.target.level : 4
      return Math.max(1, 3 - targetLevel * 0.5)
    })
  ...
  node
    .append('circle')
    .attr('r', (d) => (d.level === 1 ? 32 : d.level === 2 ? 22 : 14))
